...
getwd()
fastaFile <- readDNAStringSet("mm10-tRNAs_18.1.fa") # Insert fasta file in a variable
...
if (file.exists("tRNA_fasta.18.1.fa")){ # Check the gtf file exits if it exits remove it
  file.remove("tRNA_fasta.18.1.fa")
}
...
tRNA_fasta.18.1.fa = dataframe2fas(dffinal, file="tRNA_fasta.18.1.fa")

Line_fa<-readLines(paste("Mus_musculus.GRCm38.dna.primary_assembly.fa", sep = ""),
                     n=1)
print(Line_fa)

Line_trna<- readLines(paste("tRNA_fasta.18.1.fa", sep = ""),
                      n=1)
print(Line_trna)
