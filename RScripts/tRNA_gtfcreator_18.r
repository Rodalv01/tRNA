...
fasta_file <- readDNAStringSet("mm10-tRNAs_19.fa") # Import fastafile and keep in a variable
...
if (file.exists("tRNA_18.1.gtf")){ # Check the gtf file exits if it exits remove it
  file.remove("tRNA_18.1.gtf")
}
...
file.create("tRNA_18.txt") # Create a txt file to transfer the database
write.table(df_gtf2,file="tRNA_18.txt",row.names=FALSE,col.names = FALSE, quote=F, sep ="\t")# the quote format allow to decide which column has doublequote
file.rename("tRNA_18.txt", "tRNA_19.gtf") # Changing the extension to gtf

Line_gtf<- readLines(paste("~/Coding/Coding/Data/annotation/","Mus_musculus.GRCm38.96.gtf", sep = ""),
                     n=6)
print(Line_gtf)

Line_trna<- readLines(paste("~/Coding/Coding/Data/refgenome/","tRNA_19.gtf", sep = ""),
                      n=1)
print(Line_trna)

file.move("tRNA_19.gtf", "~/Coding/Coding/Data/annotation/", overwrite=TRUE)
